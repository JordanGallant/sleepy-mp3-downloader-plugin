...
//get client id from service worker after intercepting traffic
chrome.storage.local.get("soundcloudClientId", (result) => {
  const clientId = result.soundcloudClientId;

  if (clientId) {
    // Save to the page's localStorage
    localStorage.setItem("soundcloudClientId", clientId);
  }
});
const out = localStorage.getItem("soundcloudClientId");
const SOUNDCLOUD_CLIENT_ID = `client_id=${out}` //dynamically assigns localstorage when client id changes
console.log(SOUNDCLOUD_CLIENT_ID)
 //client id needed for authorization, can be repeated, Souncloud is dumb and bad at security
